...
using namespace riakboost::test_tools;
...
{                                                   \
    ...
    try {                                           \
        tool_usage;                                 \
    } catch( riakboost::execution_aborted const& ) {    \
        throw_ = true;                              \
    }                                               \
                                                    \
...
struct shorten_lf : public riakboost::unit_test::output::compiler_log_formatter
{
    void    print_prefix( std::ostream& output, riakboost::unit_test::const_string, std::size_t line )
    {
        output << line << ": ";
    }
};
...
{
    static riakboost::shared_ptr<output_test_stream> inst;
...
riakboost::test_tools::predicate_result
test_pred1()
{
    riakboost::test_tools::predicate_result res( false );
...
{
    riakboost::test_tools::predicate_result res( c1.m_i == c2.m_i && c1.m_id == c2.m_id );
...
TEST_CASE( test_BOOST_CHECK_PREDICATE )
{
    ...
    int j=15;
    BOOST_CHECK_PREDICATE( riakboost::bind( is_even, riakboost::bind( &foo, _1, _2 ) ), (i)(j) );
