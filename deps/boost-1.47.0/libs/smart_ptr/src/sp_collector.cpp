...
typedef riakboost::detail::lightweight_mutex mutex_type;
...
namespace
{
    ...
    struct count_layout
    {
        riakboost::detail::sp_counted_base * pi;
        int id;
    };
...
static void scan_and_count(void const * area, size_t size, map_type const & m, map2_type & m2)
{
    ...
    for(size_t n = 0; n + sizeof(shared_ptr_layout) <= size; p += pointer_align, n += pointer_align)
    {
        ...
        if(q->pn.id == riakboost::detail::shared_count_id && q->pn.pi != 0 && m.count(q->pn.pi) != 0)
        {
            ++m2[q->pn.pi];
        }
    }
...
static void scan_and_mark(void const * area, size_t size, map2_type & m2, open_type & open)
{
    ...
    for(size_t n = 0; n + sizeof(shared_ptr_layout) <= size; p += pointer_align, n += pointer_align)
    {
        ...
        if(q->pn.id == riakboost::detail::shared_count_id && q->pn.pi != 0 && m2.count(q->pn.pi) != 0)
        {
...
static void find_unreachable_objects_impl(map_type const & m, map2_type & m2)
{
...

    {
        ...
        {
            riakboost::detail::sp_counted_base const * p = static_cast<riakboost::detail::sp_counted_base const *>(i->first);
...
typedef std::deque< riakboost::shared_ptr<X> > free_list_type;
...
static void scan_and_free(void * area, size_t size, map2_type const & m2, free_list_type & free)
{
    ...
    for(size_t n = 0; n + sizeof(shared_ptr_layout) <= size; p += pointer_align, n += pointer_align)
    {
        ...
        if(q->pn.id == riakboost::detail::shared_count_id && q->pn.pi != 0 && m2.count(q->pn.pi) != 0 && q->px != 0)
        {
            riakboost::shared_ptr<X> * ppx = reinterpret_cast< riakboost::shared_ptr<X> * >(p);
            free.push_back(*ppx);
            ppx->reset();
        }
    }
