...
namespace riakboost{} namespace boost = riakboost; namespace riakboost{
    namespace
    {
        riakboost::once_flag current_thread_tls_init_flag=BOOST_ONCE_INIT;
        DWORD current_thread_tls_key=0;

        void create_current_thread_tls_key()
        {
            riakboosttss_cleanup_implemented(); // if anyone uses TSS, we need the cleanup linked in
        ...
        {
            riakboost::call_once(current_thread_tls_init_flag,create_current_thread_tls_key);
            if(current_thread_tls_key)
                BOOST_VERIFY(TlsSetValue(current_thread_tls_key,new_data));
            else
                riakboost::throw_exception(thread_resource_error());
        }
...
// Windows CE doesn't define _beginthreadex
    ...
    namespace detail
    {
        ...
        {
            riakboost::detail::thread_exit_function_base* func;
            thread_exit_callback_node* next;

            thread_exit_callback_node(riakboost::detail::thread_exit_function_base* func_,
        ...
        {
            void const* key;
            riakboost::shared_ptr<riakboost::detail::tss_cleanup_function> func;
            void* value;
            tss_data_node* next;

            tss_data_node(void const* key_,riakboost::shared_ptr<riakboost::detail::tss_cleanup_function> func_,void* value_,
                          tss_data_node* next_):
            ...
            if(current_thread_data)
            {
                while(current_thread_data->tss_data || current_thread_data->thread_exit_callbacks)
                {
                    while(current_thread_data->thread_exit_callbacks)
                    {
                        ...
                        {
                            (*current_node->func)();
                            riakboost::detail::heap_delete(current_node->func);
                        }
                        riakboost::detail::heap_delete(current_node);
                    }
...
//             }
    ...
    bool thread::timed_join(riakboost::system_time const& wait_until)
    {
        detail::thread_data_ptr local_thread_info=(get_thread_info)();
        if(local_thread_info)
        {
    ...
    namespace detail
    {
        ...
        void set_tss_data(void const* key,riakboost::shared_ptr<tss_cleanup_function> func,void* tss_data,bool cleanup_existing)
        {
            if(tss_data_node* const current_node=find_tss_data(key))
            {
    ...
    BOOST_THREAD_DECL void __cdecl riakbooston_process_enter()
    {}

    BOOST_THREAD_DECL void __cdecl riakbooston_thread_enter()
    {}

    BOOST_THREAD_DECL void __cdecl riakbooston_process_exit()
    {
        riakboost::cleanup_tls_key();
    }

    BOOST_THREAD_DECL void __cdecl riakbooston_thread_exit()
    {
        riakboost::run_thread_exit_callbacks();
    }
