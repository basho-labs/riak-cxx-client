...
namespace riakboost{} namespace boost = riakboost; namespace riakboost{
    ...
    exception_detail
        {
        std::string diagnostic_information_impl( riakboost::exception const *, std::exception const *, bool );
        }
    ...
    current_exception_diagnostic_information()
        {
        riakboost::exception const * be=current_exception_cast<riakboost::exception const>();
...
namespace riakboost{} namespace boost = riakboost; namespace riakboost{
    ...
    exception_detail
        ...
        inline
        std::string
        diagnostic_information_impl( riakboost::exception const * be, std::exception const * se, bool with_what )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef BOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<riakboost::exception const *>(se);
            if( !se )
    ...
    diagnostic_information_what( exception const & e ) throw()
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce riakboost::diagnostic_information_what()";
#ifndef BOOST_NO_EXCEPTIONS
            }
        catch(
        ... )
