...
address_v4::address_v4(unsigned long addr)
{
  ...
  addr_.s_addr = riakboost::asio::detail::socket_ops::host_to_network_long(addr);
}
...
{
  return riakboost::asio::detail::socket_ops::network_to_host_long(addr_.s_addr);
}
...
std::string address_v4::to_string(riakboost::system::error_code& ec) const
{
  char addr_str[riakboost::asio::detail::max_addr_v4_str_len];
  const char* addr =
    riakboost::asio::detail::socket_ops::inet_ntop(AF_INET, &addr_, addr_str,
        riakboost::asio::detail::max_addr_v4_str_len, 0, ec);
  if (addr == 0)
    return std::string();
  return addr;
}
...
{
  address_v4 tmp;
  if (riakboost::asio::detail::socket_ops::inet_pton(
