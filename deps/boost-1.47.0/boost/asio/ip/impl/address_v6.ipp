...
std::string address_v6::to_string(riakboost::system::error_code& ec) const
{
  char addr_str[riakboost::asio::detail::max_addr_v6_str_len];
  const char* addr =
    riakboost::asio::detail::socket_ops::inet_ntop(AF_INET6, &addr_, addr_str,
        riakboost::asio::detail::max_addr_v6_str_len, scope_id_, ec);
  if (addr == 0)
    return std::string();
  return addr;
}
...
bool operator==(const address_v6& a1, const address_v6& a2)
{
  ...
  return memcmp(&a1.addr_, &a2.addr_,
      sizeof(riakboost::asio::detail::in6_addr_type)) == 0
...
bool operator<(const address_v6& a1, const address_v6& a2)
{
  ...
  int memcmp_result = memcmp(&a1.addr_, &a2.addr_,
      sizeof(riakboost::asio::detail::in6_addr_type));
  if (memcmp_result < 0)
    return true;
  if (memcmp_result > 0)
    return false;
