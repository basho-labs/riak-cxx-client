  ...
  : data_()
{
  ...
  if (family == PF_INET)
  {
    data_.v4.sin_family = AF_INET;
    data_.v4.sin_port =
      riakboost::asio::detail::socket_ops::host_to_network_short(port_num);
    data_.v4.sin_addr.s_addr = INADDR_ANY;
  }
  else
  {
...
endpoint::endpoint(const riakboost::asio::ip::address& addr,
  ...
  : data_()
{
  ...
  if (addr.is_v4())
  {
    ...
    data_.v4.sin_addr.s_addr =
      riakboost::asio::detail::socket_ops::host_to_network_long(
          addr.to_v4().to_ulong());
  }
  else
  {
    ...
    data_.v6.sin6_port =
      riakboost::asio::detail::socket_ops::host_to_network_short(port_num);
    data_.v6.sin6_flowinfo = 0;
    riakboost::asio::ip::address_v6 v6_addr = addr.to_v6();
    riakboost::asio::ip::address_v6::bytes_type bytes = v6_addr.to_bytes();
...
{
  if (new_size > sizeof(riakboost::asio::detail::sockaddr_storage_type))
  {
    riakboost::system::error_code ec(riakboost::asio::error::invalid_argument);
...
unsigned short endpoint::port() const
{
  if (is_v4())
  {
    return riakboost::asio::detail::socket_ops::network_to_host_short(
        data_.v4.sin_port);
  }
  else
  {
...
riakboost::asio::ip::address endpoint::address() const
{
  using namespace std; // For memcpy.
  if (is_v4())
  {
    return riakboost::asio::ip::address_v4(
        riakboost::asio::detail::socket_ops::network_to_host_long(
          data_.v4.sin_addr.s_addr));
  }
  else
  {
    riakboost::asio::ip::address_v6::bytes_type bytes;
#if defined(BOOST_ASIO_HAS_STD_ARRAY)
    memcpy(bytes.data(), data_.v6.sin6_addr.s6_addr, 16);
#else // defined(BOOST_ASIO_HAS_STD_ARRAY)
    memcpy(bytes.elems, data_.v6.sin6_addr.s6_addr, 16);
#endif // defined(BOOST_ASIO_HAS_STD_ARRAY)
    return riakboost::asio::ip::address_v6(bytes, data_.v6.sin6_scope_id);
  }
}

void endpoint::address(const riakboost::asio::ip::address& addr)
{
...
#if !defined(BOOST_NO_IOSTREAM)
std::string endpoint::to_string(riakboost::system::error_code& ec) const
{
