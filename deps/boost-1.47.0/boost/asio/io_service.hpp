 ...
 *
 * @li riakboost::asio::ip::tcp::socket
 * @li riakboost::asio::ip::tcp::acceptor
 * @li riakboost::asio::ip::udp::socket
 * @li riakboost::asio::deadline_timer.
 *
 ...
 *
 * @code
 * riakboost::asio::io_service io_service;
 * ...
 * for (;;)
 * {
 *   try
 ...
 * creating an object of type riakboost::asio::io_service::work:
 *
 * @code riakboost::asio::io_service io_service;
 * riakboost::asio::io_service::work work(io_service);
 * ... @endcode
 *
 ...
 *
 * @code riakboost::asio::io_service io_service;
 * auto_ptr<riakboost::asio::io_service::work> work(
 *     new riakboost::asio::io_service::work(io_service));
 * ...
...
public:
   ...
   * without blocking.
   */
  BOOST_ASIO_DECL std::size_t run(riakboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t run_one(riakboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t poll(riakboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t poll_one(riakboost::system::error_code& ec);
   ...
   * @code io_service.dispatch(riakboost::bind(f, a1, ... an)); @endcode
   */
   ...
   *
   * @throws riakboost::system::system_error Thrown on failure. If the notification
   ...
   * function:
   * @code my_io_service.notify_fork(riakboost::asio::io_service::fork_prepare);
   * if (fork() == 0)
   * {
   *   // This is the child process.
   *   my_io_service.notify_fork(riakboost::asio::io_service::fork_child);
   * }
   * else
   * {
   *   // This is the parent process.
   *   my_io_service.notify_fork(riakboost::asio::io_service::fork_parent);
   * } @endcode
   *
   ...
   */
  BOOST_ASIO_DECL void notify_fork(riakboost::asio::io_service::fork_event event);
   ...
   *
   * @throws riakboost::asio::service_already_exists Thrown if a service of the
   * given type is already present in the io_service.
   *
   * @throws riakboost::asio::invalid_service_owner Thrown if the service's owning
...
private:
  ...
  riakboost::asio::detail::service_registry* service_registry_;
...
public:
   ...
   */
  explicit work(riakboost::asio::io_service& io_service);
  ...
  riakboost::asio::io_service& get_io_service();
...
private:
  ...
  // The io_service.
  riakboost::asio::io_service& io_service_;
};
...
protected:
   ...
   */
  BOOST_ASIO_DECL service(riakboost::asio::io_service& owner);
...
private:
  ...
  BOOST_ASIO_DECL virtual void fork_service(
      riakboost::asio::io_service::fork_event event);

  friend class riakboost::asio::detail::service_registry;
  struct key
  {
    key() : type_info_(0), id_(0) {}
    const std::type_info* type_info_;
    const riakboost::asio::io_service::id* id_;
  } key_;

  riakboost::asio::io_service& owner_;
  service* next_;
};
...
class service_id
  : public riakboost::asio::io_service::id
{
};
...
template <typename Type>
class service_base
  : public riakboost::asio::io_service::service
{
public:
  static riakboost::asio::detail::service_id<Type> id;

  // Constructor.
  service_base(riakboost::asio::io_service& io_service)
    : riakboost::asio::io_service::service(io_service)
  {
  }
};

template <typename Type>
riakboost::asio::detail::service_id<Type> service_base<Type>::id;
