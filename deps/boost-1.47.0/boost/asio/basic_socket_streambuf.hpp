...
template <typename Protocol,
    ...
    typename Time = riakboost::posix_time::ptime,
    typename TimeTraits = riakboost::asio::time_traits<Time>,
    typename TimerService = deadline_timer_service<Time, TimeTraits> >
class basic_socket_streambuf
  : public std::streambuf,
    private riakboost::base_from_member<io_service>,
...
public:
  ...
  basic_socket_streambuf()
    : basic_socket<Protocol, StreamSocketService>(
        riakboost::base_from_member<riakboost::asio::io_service>::member),
      unbuffered_(false),
      timer_service_(0),
      ...
      const endpoint_type& endpoint)
  {
    ...
    {
      ec_ = riakboost::asio::error::operation_aborted;
      return 0;
    }
   ...
   * buffer.
   */
  const riakboost::system::error_code& puberror() const
  {
    return error();
  }
   ...
   * riakboost::asio::error::operation_aborted.
   *
  ...
  void expires_at(const time_type& expiry_time)
  {
    ...
    riakboost::asio::detail::throw_error(ec, "expires_at");
  ...
  void expires_from_now(const duration_type& expiry_time)
  {
    ...
    riakboost::asio::detail::throw_error(ec, "expires_from_now");
...
protected:
  int_type underflow()
  {
    if (gptr() == egptr())
    {
      ...
      this->get_service().async_receive(this->get_implementation(),
          riakboost::asio::buffer(riakboost::asio::buffer(get_buffer_) + putback_max),
          0, handler);
  ...
  int_type overflow(int_type c)
  {
    if (unbuffered_)
    {
      ...
      else
      {
        ...
        this->get_service().async_send(this->get_implementation(),
            riakboost::asio::buffer(&ch, sizeof(char_type)), 0, handler);
    ...
    else
    {
      ...
      riakboost::asio::const_buffer buffer =
        riakboost::asio::buffer(pbase(), pptr() - pbase());
      while (riakboost::asio::buffer_size(buffer) > 0)
      {
        if (timer_state_ == timer_has_expired)
        {
          ec_ = riakboost::asio::error::operation_aborted;
        ...
        io_handler handler = { this };
        this->get_service().async_send(this->get_implementation(),
            riakboost::asio::buffer(buffer), 0, handler);
   ...
   * buffer.
   */
  virtual const riakboost::system::error_code& error() const
  {
    return ec_;
  }
...
private:
  ...
  void resolve_and_connect(const ResolverQuery& query)
  {
    ...
    {
      iterator_type end;
      ec_ = riakboost::asio::error::host_not_found;
      while (ec_ && i != end)
      {
        this->basic_socket<Protocol, StreamSocketService>::close(ec_);
  ...
  struct timer_handler
  {
    ...
    void operator()(const riakboost::system::error_code&)
    {
  ...
  void start_timer()
  {
    if (timer_state_ != timer_is_pending)
    {
      ...
      handler(riakboost::system::error_code());
    }
  }

  enum { putback_max = 8 };
  enum { buffer_size = 512 };
  riakboost::asio::detail::array<char, buffer_size> get_buffer_;
  riakboost::asio::detail::array<char, buffer_size> put_buffer_;
  bool unbuffered_;
  riakboost::system::error_code ec_;
