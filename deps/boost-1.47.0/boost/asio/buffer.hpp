 ...
 *
 * @code riakboost::asio::mutable_buffer b1 = ...;
 * std::size_t s1 = riakboost::asio::buffer_size(b1);
 * unsigned char* p1 = riakboost::asio::buffer_cast<unsigned char*>(b1);
 * @endcode
 *
 * The riakboost::asio::buffer_cast function permits violations of type safety, so
...
public:
...
#if defined(BOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
  mutable_buffer(void* data, std::size_t size,
      riakboost::function<void()> debug_check)
    : data_(data),
      size_(size),
  ...
  const riakboost::function<void()>& get_debug_check() const
  {
    return debug_check_;
  }
#endif // BOOST_ASIO_ENABLE_BUFFER_DEBUGGING
...
private:
  friend void* riakboost::asio::detail::buffer_cast_helper(
      const mutable_buffer& b);
  friend std::size_t riakboost::asio::detail::buffer_size_helper(
      const mutable_buffer& b);
...
#if defined(BOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
  riakboost::function<void()> debug_check_;
#endif // BOOST_ASIO_ENABLE_BUFFER_DEBUGGING
};
 ...
 * and @ref buffer_cast functions:
 *
 * @code riakboost::asio::const_buffer b1 = ...;
 * std::size_t s1 = riakboost::asio::buffer_size(b1);
 * const unsigned char* p1 = riakboost::asio::buffer_cast<const unsigned char*>(b1);
 * @endcode
 *
 * The riakboost::asio::buffer_cast function permits violations of type safety, so
 * uses of it in application code should be carefully considered.
...
public:
  ...
  const_buffer(const mutable_buffer& b)
    : data_(riakboost::asio::detail::buffer_cast_helper(b)),
      size_(riakboost::asio::detail::buffer_size_helper(b))
#if defined(BOOST_ASIO_ENABLE_BUFFER_DEBUGGING)
...
private:
  friend const void* riakboost::asio::detail::buffer_cast_helper(
...
/** @defgroup buffer_size riakboost::asio::buffer_size
 *
 * @brief The riakboost::asio::buffer_size function determines the total number of
...
/** @defgroup buffer_cast riakboost::asio::buffer_cast
 *
 * @brief The riakboost::asio::buffer_cast function is used to obtain a pointer to
 ...
 * @code riakboost::asio::mutable_buffer b2 = ...;
 * unsigned char* p2 = riakboost::asio::buffer_cast<unsigned char*>(b2);
 * @endcode
 *
 * The riakboost::asio::buffer_cast function permits violations of type safety, so
 * uses of it in application code should be carefully considered.
...
/** @defgroup buffer riakboost::asio::buffer
 *
 * @brief The riakboost::asio::buffer function is used to create a buffer object to
 ...
 * specified size:
 *
 * @code sock.send(riakboost::asio::buffer(data, size)); @endcode
 *
 * In the above example, the return value of riakboost::asio::buffer meets the
 * requirements of the ConstBufferSequence concept so that it may be directly
 * passed to the socket's write function. A buffer created for modifiable
 * memory also meets the requirements of the MutableBufferSequence concept.
 *
 * An individual buffer may be created from a builtin array, std::vector,
 * std::array or riakboost::array of POD elements. This helps prevent buffer
 * overruns by automatically determining the size of the buffer:
 *
 * @code char d1[128];
 * size_t bytes_transferred = sock.receive(riakboost::asio::buffer(d1));
 *
 * std::vector<char> d2(128);
 * bytes_transferred = sock.receive(riakboost::asio::buffer(d2));
 *
 * std::array<char, 128> d3;
 * bytes_transferred = sock.receive(riakboost::asio::buffer(d3));
 *
 * riakboost::array<char, 128> d4;
 * bytes_transferred = sock.receive(riakboost::asio::buffer(d4)); @endcode
 *
 ...
 * unsigned char* p1 = riakboost::asio::buffer_cast<unsigned char*>(b1);
 *
 * riakboost::asio::const_buffer b2 = ...;
 * std::size_t s2 = riakboost::asio::buffer_size(b2);
 * const void* p2 = riakboost::asio::buffer_cast<const void*>(b2); @endcode
 *
 * The riakboost::asio::buffer_cast function permits violations of type safety, so
 * uses of it in application code should be carefully considered.
 *
 ...
 *
 * vector<unsigned char> data(riakboost::asio::buffer_size(buffers));
 * riakboost::asio::buffer_copy(riakboost::asio::buffer(data), buffers); @endcode
 *
 ...
 *
 * For the riakboost::asio::buffer overloads that accept an argument of type
 ...
 * follows:
 *
 * @code riakboost::array<char, 6> a = { 'a', 'b', 'c', 'd', 'e' }; @endcode
 *
 * A buffer object @c b1 created using:
 *
 * @code b1 = riakboost::asio::buffer(a); @endcode
 *
 * represents the entire array, <tt>{ 'a', 'b', 'c', 'd', 'e' }</tt>. An
 * optional second argument to the riakboost::asio::buffer function may be used to
 * limit the size, in bytes, of the buffer:
 *
 * @code b2 = riakboost::asio::buffer(a, 3); @endcode
 *
 ...
 *
 * @code b4 = riakboost::asio::buffer(b1 + 1, 3); @endcode
 *
 ...
 * char d1[128];
 * std::vector<char> d2(128);
 * riakboost::array<char, 128> d3;
 *
 * riakboost::array<mutable_buffer, 3> bufs1 = {
 *   riakboost::asio::buffer(d1),
 *   riakboost::asio::buffer(d2),
 *   riakboost::asio::buffer(d3) };
 * bytes_transferred = sock.receive(bufs1);
 *
 * std::vector<const_buffer> bufs2;
 * bufs2.push_back(riakboost::asio::buffer(d1));
 * bufs2.push_back(riakboost::asio::buffer(d2));
 * bufs2.push_back(riakboost::asio::buffer(d3));
...
//
//   unspecified buffer(riakboost::array<PodType, N>& array ...);
//
// and
//
//   unspecified buffer(riakboost::array<const PodType, N>& array ...);
//
...
struct buffer_types
  : public buffer_types_base<riakboost::is_const<PodType>::value>
{
};
...
buffer(riakboost::array<PodType, N>& data)
{
  typedef typename riakboost::asio::detail::buffer_types<PodType>::buffer_type
    buffer_type;
  typedef typename riakboost::asio::detail::buffer_types<PodType>::container_type
    container_type;
...
template <typename PodType, std::size_t N>
inline typename detail::buffer_types<PodType>::container_type
buffer(riakboost::array<PodType, N>& data, std::size_t max_size_in_bytes)
{
  typedef typename riakboost::asio::detail::buffer_types<PodType>::buffer_type
    buffer_type;
  typedef typename riakboost::asio::detail::buffer_types<PodType>::container_type
 ...
 */
template <typename PodType, std::size_t N>
inline mutable_buffers_1 buffer(riakboost::array<PodType, N>& data)
{
  return mutable_buffers_1(
 ...
 */
template <typename PodType, std::size_t N>
inline const_buffers_1 buffer(riakboost::array<const PodType, N>& data)
{
  return const_buffers_1(
 ...
 */
template <typename PodType, std::size_t N>
inline const_buffers_1 buffer(const riakboost::array<PodType, N>& data)
{
  return const_buffers_1(
      const_buffer(data.data(), data.size() * sizeof(PodType)));
}
...
/** @defgroup buffer_copy riakboost::asio::buffer_copy
 *
 * @brief The riakboost::asio::buffer_copy function is used to copy bytes from a
