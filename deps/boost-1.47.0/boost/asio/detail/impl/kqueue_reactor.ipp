...
kqueue_reactor::kqueue_reactor(riakboost::asio::io_service& io_service)
  : riakboost::asio::detail::service_base<kqueue_reactor>(io_service),
...
void kqueue_reactor::fork_service(riakboost::asio::io_service::fork_event fork_ev)
{
  if (fork_ev == riakboost::asio::io_service::fork_child)
  {
        ...
        state != 0; state = state->next_)
    {
      ...
      {
        riakboost::system::error_code error(errno,
            riakboost::asio::error::get_system_category());
        riakboost::asio::detail::throw_error(error);
      }
    }
  }
}
    ...
    reactor_op* op, bool allow_speculative)
{
  ...
  if (first)
  {
    ...
    {
      op->ec_ = riakboost::system::error_code(errno,
          riakboost::asio::error::get_system_category());
        ...
        if (events[i].filter == filter[j])
        {
          if (j != except_op || events[i].flags & EV_OOBAND)
          {
            while (reactor_op* op = descriptor_data->op_queue_[j].front())
            {
              ...
              {
                op->ec_ = riakboost::system::error_code(events[i].data,
                    riakboost::asio::error::get_system_category());
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    riakboost::system::error_code ec(errno,
        riakboost::asio::error::get_system_category());
    riakboost::asio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
