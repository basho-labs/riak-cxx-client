...
dev_poll_reactor::dev_poll_reactor(riakboost::asio::io_service& io_service)
  : riakboost::asio::detail::service_base<dev_poll_reactor>(io_service),
...
void dev_poll_reactor::fork_service(riakboost::asio::io_service::fork_event fork_ev)
{
  if (fork_ev == riakboost::asio::io_service::fork_child)
  {
...
{
  riakboost::asio::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, riakboost::asio::error::operation_aborted);
}
...
void dev_poll_reactor::run(bool block, op_queue<operation>& ops)
{
  ...
  if (events_size > 0)
  {
    ...
    {
      riakboost::system::error_code ec = riakboost::system::error_code(
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (result != sizeof(ev))
        {
          riakboost::system::error_code ec(errno,
...
int dev_poll_reactor::do_dev_poll_create()
{
  ...
  if (fd == -1)
  {
    riakboost::system::error_code ec(errno,
        riakboost::asio::error::get_system_category());
    riakboost::asio::detail::throw_error(ec, "/dev/poll");
  }
  return fd;
}
