...
win_iocp_io_service::win_iocp_io_service(riakboost::asio::io_service& io_service)
  : riakboost::asio::detail::service_base<win_iocp_io_service>(io_service),
    iocp_(),
    outstanding_work_(0),
    stopped_(0),
    shutdown_(0),
...
void win_iocp_io_service::init(size_t concurrency_hint)
{
  ...
  if (!iocp_.handle)
  {
    ...
    riakboost::asio::detail::throw_error(ec, "iocp");
  }
}
...
riakboost::system::error_code win_iocp_io_service::register_handle(
...
size_t win_iocp_io_service::run(riakboost::system::error_code& ec)
{
...
size_t win_iocp_io_service::run_one(riakboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_service::poll(riakboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_service::poll_one(riakboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
void win_iocp_io_service::stop()
{
  if (::InterlockedExchange(&stopped_, 1) == 0)
  {
    if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0))
    {
      ...
      riakboost::system::error_code ec(last_error,
          riakboost::asio::error::get_system_category());
      riakboost::asio::detail::throw_error(ec, "pqcs");
    }
  }
}
...
void win_iocp_io_service::on_completion(win_iocp_operation* op,
    const riakboost::system::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
...
size_t win_iocp_io_service::do_one(bool block, riakboost::system::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      riakboost::system::error_code result_ec(last_error,
          riakboost::asio::error::get_system_category());
      ...
      {
        result_ec = riakboost::system::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<riakboost::system::error_category*>(op->Internal));
...
void win_iocp_io_service::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!waitable_timer_.handle)
  {
    ...
    if (waitable_timer_.handle == 0)
    {
      ...
      riakboost::system::error_code ec(last_error,
          riakboost::asio::error::get_system_category());
      riakboost::asio::detail::throw_error(ec, "timer");
    }
