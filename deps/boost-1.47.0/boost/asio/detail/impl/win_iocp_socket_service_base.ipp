...
void win_iocp_socket_service_base::shutdown_service()
{
  ...
  riakboost::asio::detail::mutex::scoped_lock lock(mutex_);
    ...
    win_iocp_socket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    riakboost::asio::detail::mutex::scoped_lock lock(other_service.mutex_);
...
riakboost::system::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type& impl,
...
riakboost::system::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type& impl,
    riakboost::system::error_code& ec)
{
  if (!is_open(impl))
        ...
        ::GetModuleHandleA("KERNEL32"), "CancelIoEx"))
  {
    ...
    if (!cancel_io_ex(sock_as_handle, 0))
    {
      ...
      {
        ec = riakboost::system::error_code(last_error,
...
riakboost::system::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int family, int type, int protocol, riakboost::system::error_code& ec)
{
  if (is_open(impl))
  {
...
riakboost::system::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int type, socket_type native_socket, riakboost::system::error_code& ec)
{
  if (is_open(impl))
  {
    ec = riakboost::asio::error::already_open;
    ...
    socket_base::message_flags flags, bool noop, operation* op)
{
  ...
  if (noop)
    ...
    iocp_service_.on_completion(op, riakboost::asio::error::bad_descriptor);
  else
  {
    ...
    int protocol, void* output_buffer, DWORD address_length, operation* op)
{
  ...
  if (!is_open(impl))
    ...
    iocp_service_.on_completion(op, riakboost::asio::error::already_open);
  else
  {
    riakboost::system::error_code ec;
    ...
    int op_type, reactor_op* op)
{
  ...
  if (is_open(impl))
  {
    ...
    op->ec_ = riakboost::asio::error::bad_descriptor;
    ...
    reactor_op* op, const socket_addr_type* addr, std::size_t addrlen)
{
        ...
        impl.socket_, impl.state_, true, op->ec_))
  {
    ...
    {
      if (op->ec_ == riakboost::asio::error::in_progress
          || op->ec_ == riakboost::asio::error::would_block)
      {
        op->ec_ = riakboost::system::error_code();
