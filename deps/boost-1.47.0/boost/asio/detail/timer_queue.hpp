...
public:
  ...
  virtual long wait_duration_msec(long max_duration) const
  {
    ...
    riakboost::posix_time::time_duration duration = Time_Traits::to_posix_duration(
        Time_Traits::subtract(heap_[0].time_, Time_Traits::now()));

    if (duration > riakboost::posix_time::milliseconds(max_duration))
      duration = riakboost::posix_time::milliseconds(max_duration);
    else if (duration <= riakboost::posix_time::milliseconds(0))
      duration = riakboost::posix_time::milliseconds(0);
    else if (duration < riakboost::posix_time::milliseconds(1))
      duration = riakboost::posix_time::milliseconds(1);
  ...
  virtual long wait_duration_usec(long max_duration) const
  {
    ...
    if (duration > riakboost::posix_time::microseconds(max_duration))
      duration = riakboost::posix_time::microseconds(max_duration);
    else if (duration <= riakboost::posix_time::microseconds(0))
      duration = riakboost::posix_time::microseconds(0);
    else if (duration < riakboost::posix_time::microseconds(1))
      duration = riakboost::posix_time::microseconds(1);
      ...
      std::size_t max_cancelled = (std::numeric_limits<std::size_t>::max)())
  {
    ...
    if (timer.prev_ != 0 || &timer == timers_)
    {
      ...
      {
        op->ec_ = riakboost::asio::error::operation_aborted;
        timer.op_queue_.pop();
        ops.push(op);
        ++num_cancelled;
      }
...
private:
  ...
  // Determine if the specified absolute time is positive infinity.
  static bool is_positive_infinity(const riakboost::posix_time::ptime& time)
  {
    return time == riakboost::posix_time::pos_infin;
  }
...
struct forwarding_posix_time_traits : time_traits<riakboost::posix_time::ptime> {};

// Template specialisation for the commonly used instantation.
template <>
class timer_queue<time_traits<riakboost::posix_time::ptime> >
  : public timer_queue_base
{
public:
  // The time type.
  typedef riakboost::posix_time::ptime time_type;

  // The duration type.
  typedef riakboost::posix_time::time_duration duration_type;
