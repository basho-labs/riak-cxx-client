...
template< typename T, typename name = ::riakboost::mpl::aux::has_xxx_tag > \
struct BOOST_PP_CAT(trait,_impl) : T \
{ \
    static riakboost::mpl::aux::no_tag \
    test(void(*)(::riakboost::mpl::aux::has_xxx_tag)); \
    \
    static riakboost::mpl::aux::yes_tag test(...); \
    \
    BOOST_STATIC_CONSTANT(bool, value = \
          sizeof(test(static_cast<void(*)(name)>(0))) \
            != sizeof(riakboost::mpl::aux::no_tag) \
        ); \
    typedef riakboost::mpl::bool_<value> type; \
}; \
\
template< typename T, typename fallback_ = riakboost::mpl::bool_<default_> > \
struct trait \
    : riakboost::mpl::if_c< \
          riakboost::mpl::aux::msvc_is_incomplete<T>::value \
        , riakboost::mpl::bool_<false> \
...
{ \
    BOOST_STATIC_CONSTANT(bool, value = false); \
    typedef riakboost::mpl::bool_<false> type; \
}; \
/**/
...
{ \
    ...
    { \
        template < class U > \
        static int  resolve( riakboost::mpl::aux::type_wrapper<U> const volatile * \
                           , riakboost::mpl::aux::type_wrapper<typename U::name >* = 0 ); \
        static char resolve( ... ); \
    }; \
    typedef riakboost::mpl::aux::type_wrapper<T> t_; \
    BOOST_STATIC_CONSTANT( bool, value = ( sizeof( trait_tester_impl::resolve( static_cast< t_ * >(0) ) ) == sizeof(int) ) ); \
}; \
template< typename T, typename fallback_ = riakboost::mpl::bool_<default_> > \
struct trait           \
{                      \
    BOOST_STATIC_CONSTANT( bool, value = (trait_tester< T, riakboost::is_class< T >::value >::value) );     \
    typedef riakboost::mpl::bool_< trait< T, fallback_ >::value > type; \
};
...
{ \
    ...
    { \
        template< typename U > \
        static riakboost::mpl::aux::yes_tag test( \
              riakboost::mpl::aux::type_wrapper<U> const volatile* \
            , riakboost::mpl::aux::type_wrapper<BOOST_MSVC_TYPENAME U::name>* = 0 \
            ); \
    \
        static riakboost::mpl::aux::no_tag test(...); \
    }; \
    \
    typedef riakboost::mpl::aux::type_wrapper<T> t_; \
    BOOST_STATIC_CONSTANT(bool, value = \
          sizeof(gcc_3_2_wknd::test(static_cast<t_*>(0))) \
            == sizeof(riakboost::mpl::aux::yes_tag) \
        ); \
    typedef riakboost::mpl::bool_<value> type; \
}; \
/**/
...
#     define BOOST_MPL_HAS_MEMBER_MULTI_ACCEPT(z, n, args) \
        template< typename V > \
        static riakboost::mpl::aux::yes_tag \
        BOOST_MPL_HAS_MEMBER_INTROSPECTION_TEST_NAME(args)( \
            riakboost::mpl::aux::type_wrapper< V > const volatile* \
...
#       define BOOST_MPL_HAS_MEMBER_TEST(args) \
          sizeof( \
              BOOST_MPL_HAS_MEMBER_INTROSPECTION_TEST_NAME(args)( \
                  static_cast< riakboost::mpl::aux::type_wrapper< U >* >(0) \
              ) \
          ) == sizeof(riakboost::mpl::aux::yes_tag) \
        /**/
#     else
...
#   define BOOST_MPL_HAS_MEMBER_IMPLEMENTATION( \
      ...
      template< \
          typename T \
        , typename fallback_ \
              = riakboost::mpl::bool_< BOOST_PP_ARRAY_ELEM(3, args) > \
      > \
...
#   define BOOST_MPL_HAS_XXX_TEMPLATE_NAMED_DEF(trait, name, default_) \
      template< typename T \
              , typename fallback_ = riakboost::mpl::bool_< default_ > > \
      struct trait { \
          BOOST_STATIC_CONSTANT(bool, value = fallback_::value); \
          typedef fallback_ type; \
      }; \
