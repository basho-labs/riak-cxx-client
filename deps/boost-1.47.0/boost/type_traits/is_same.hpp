...
struct is_same_impl
{
    enum { value = riakboost::detail::is_same_part_1<T1>::template part_2<T2>::value };
};

#else // generic "no-partial-specialization" version

template <typename T>
::riakboost::type_traits::yes_type
BOOST_TT_DECL is_same_tester(T*, T*);

::riakboost::type_traits::no_type
...
struct is_same_impl
{
   ...
   BOOST_STATIC_CONSTANT(bool, value =
      (::riakboost::type_traits::ice_and<
         (sizeof(type_traits::yes_type) == sizeof(riakboost::detail::is_same_tester(&t,&u))),
         (::riakboost::is_reference<T>::value == ::riakboost::is_reference<U>::value),
         (sizeof(T) == sizeof(U))
        >::value));
};
...
BOOST_TT_AUX_BOOL_TRAIT_DEF2(is_same,T,U,(::riakboost::detail::is_same_impl<T,U>::value))
