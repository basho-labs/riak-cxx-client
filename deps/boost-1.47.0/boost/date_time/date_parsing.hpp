  ...
  convert_to_lower(std::string inp)
  {
    ...
    month_str_to_ushort(std::string const& s) {
      if((s.at(0) >= '0') && (s.at(0) <= '9')) {
        return riakboost::lexical_cast<unsigned short>(s);
      }
      else {
    ...
    parse_date(const std::string& s, int order_spec = ymd_order_iso) {
      ...
      typedef riakboost::char_separator<char, traits_type> char_separator_type;
      typedef riakboost::tokenizer<char_separator_type,
      ...
      for(tokenizer_iterator beg=tok.begin();
        ...
        switch(spec_str.at(pos)) {
          case 'y':
          {
            year = riakboost::lexical_cast<unsigned short>(*beg);
            break;
          }
          case 'm':
          {
          ...
          case 'd':
          {
            day = riakboost::lexical_cast<unsigned short>(*beg);
            break;
          }
        } //switch
      }
    ...
    parse_undelimited_date(const std::string& s) {
      ...
      riakboost::offset_separator osf(offsets, offsets+3, false, false);

      typedef typename riakboost::tokenizer<riakboost::offset_separator,
                                        std::basic_string<char>::const_iterator,
      ...
      for(typename tokenizer_type::iterator ti=tok.begin(); ti!=tok.end();++ti) {
        unsigned short i = riakboost::lexical_cast<unsigned short>(*ti);
        switch(pos) {
        case 0: y = i; break;
        case 1: m = i; break;
        case 2: d = i; break;
    ...
    from_simple_string_type(const std::basic_string<charT>& s){
      ...
      typedef typename riakboost::char_separator<charT, traits_type> char_separator;
      typedef typename riakboost::tokenizer<char_separator,
