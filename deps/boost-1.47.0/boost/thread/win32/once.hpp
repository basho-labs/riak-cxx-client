...
#define BOOST_ONCE_INIT {0,0}
...
#ifdef BOOST_NO_ANSI_APIS                        
            return ::riakboost::detail::win32::OpenEventW(
#else
            return ::riakboost::detail::win32::OpenEventA(
#endif
                ::riakboost::detail::win32::synchronize | 
                ::riakboost::detail::win32::event_modify_state,
                false,
                mutex_name);
        }
...
#ifdef BOOST_NO_ANSI_APIS                        
            return ::riakboost::detail::win32::CreateEventW(
#else
            return ::riakboost::detail::win32::CreateEventA(
#endif
                0,::riakboost::detail::win32::manual_reset_event,
                ::riakboost::detail::win32::event_initially_reset,
                mutex_name);
        }
    }
    ...
    void call_once(once_flag& flag,Function f)
    {
        ...
        while((status=::riakboost::detail::interlocked_read_acquire(&flag.status))
              !=function_complete_flag_value)
        {
            ...
            if(!status)
            {
                try
                {
                    ...
                    if(event_handle)
                    {
                        ::riakboost::detail::win32::ResetEvent(event_handle);
                    }
                    f();
                    if(!counted)
                    {
                    ...
                    if(!event_handle && 
                       (::riakboost::detail::interlocked_read_acquire(&flag.count)>1))
                    {
                        event_handle=detail::create_once_event(mutex_name,&flag);
                    }
                    if(event_handle)
                    {
                        ::riakboost::detail::win32::SetEvent(event_handle);
                    }
                    break;
                }
                catch(...)
            ...
            if(!counted)
            {
                BOOST_INTERLOCKED_INCREMENT(&flag.count);
                counted=true;
                status=::riakboost::detail::interlocked_read_acquire(&flag.status);
            ...
            BOOST_VERIFY(!::riakboost::detail::win32::WaitForSingleObject(
                             event_handle,::riakboost::detail::win32::infinite));
        }
    }
}
