...
#define BOOST_DEFINE_HAS_MEMBER_CALLED(member_name)                     \
        template<typename T, bool=riakboost::is_class<T>::value>            \
    ...
    public:
        ...
        bool timed_lock(::riakboost::system_time const& absolute_time)
        {
            is_locked=m->timed_lock(absolute_time);
            return is_locked;
        }
        bool timed_lock(::riakboost::xtime const& absolute_time)
        {
            is_locked=m->timed_lock(absolute_time);
            return is_locked;
        }
    ...
    public:
...
#else
        void swap(riakboost::detail::thread_move_t<shared_lock<Mutex> > other)
        {
            std::swap(m,other->m);
            std::swap(is_locked,other->is_locked);
        }
        ...
        bool timed_lock(riakboost::system_time const& target_time)
        {
            if(owns_lock())
            {
                riakboost::throw_exception(riakboost::lock_error());
    ...
    namespace detail
    {
        ...
        {
            riakboost::unique_lock<MutexType1> l1(m1,riakboost::try_to_lock);
            if(!l1)
            {
                return 1;
            }
        ...
        {
            riakboost::unique_lock<MutexType1> l1(m1);
            if(!m2.try_lock())
            {
                return 1;
            }
