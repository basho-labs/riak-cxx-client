...
#define BOOST_ITEST_EPOINT( description ) \
    ::riakboost::itest::manager::instance().exception_point( BOOST_TEST_L(__FILE__), __LINE__, description )
/**/
...
#define BOOST_ITEST_DPOINT() \
    ::riakboost::itest::manager::instance().decision_point( BOOST_TEST_L(__FILE__), __LINE__ )
/**/
...
#define BOOST_ITEST_SCOPE( scope_name ) \
    ::riakboost::itest::scope_guard itest_scope_guard ## __LINE__( BOOST_TEST_L(__FILE__), __LINE__, BOOST_STRINGIZE(scope_name) )
/**/
...
#define BOOST_ITEST_NEW( type_name ) \
    new ( ::riakboost::itest::location( BOOST_TEST_L(__FILE__), __LINE__ ) ) type_name
/**/
...
#define BOOST_ITEST_DATA_FLOW( v ) \
    ::riakboost::itest::manager::instance().generic_data_flow( v )
/**/
...
#define BOOST_ITEST_RETURN( type, default_value ) \
    ::riakboost::itest::manager::instance().generic_return<type>( default_value )
/**/
...
#define BOOST_ITEST_MOCK_FUNC( function_name )          \
    ...
    return ::riakboost::itest::mock_object<>::prototype();  \
/**/
...
inline void*
operator new( std::size_t s, ::riakboost::itest::location const& l )
{
    void* res = std::malloc(s ? s : 1);

    if( res )
        ::riakboost::itest::manager::instance().allocated( l.m_file_name, l.m_line_num, res, s );
    else
        throw std::bad_alloc();
...
inline void
operator delete( void* p, ::riakboost::itest::location const& )
{
    ::riakboost::itest::manager::instance().freed( p );
