...
public:
    // Constructor
    explicit    translate_exception_base( riakboost::scoped_ptr<translate_exception_base>& next )
    {
        next.swap( m_next );
    }
...
protected:
    // Data members
    riakboost::scoped_ptr<translate_exception_base> m_next;
};
...
public:
    ...
    void        register_exception_translator( ExceptionTranslator const& tr, riakboost::type<Exception>* = 0 );
...
private:
    ...
    // Data members
    riakboost::scoped_ptr<detail::translate_exception_base> m_custom_translators;
    riakboost::scoped_array<char>                           m_alt_stack;
}; // execution_monitor
...
{
    typedef riakboost::scoped_ptr<translate_exception_base> base_ptr;
public:
    ...
    int operator()( unit_test::callback0<int> const& F )
    {
        try {
            ...
            m_translator( e );
            return riakboost::exit_exception_failure;
        }
    }
...
template<typename Exception, typename ExceptionTranslator>
void
execution_monitor::register_exception_translator( ExceptionTranslator const& tr, riakboost::type<Exception>* )
{
...
#define BOOST_TEST_SYS_ASSERT( exp ) if( (exp) ) ; else throw ::riakboost::system_error( BOOST_STRINGIZE( exp ) )
