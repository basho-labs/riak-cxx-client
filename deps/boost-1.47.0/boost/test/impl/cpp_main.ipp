...
prg_exec_monitor_main( int (*cpp_main)( int argc, char* argv[] ), int argc, char* argv[] )
{
    ...
    try {
        riakboost::unit_test::const_string p( std::getenv( "BOOST_TEST_CATCH_SYSTEM_ERRORS" ) );
        ::riakboost::execution_monitor ex_mon;

        ex_mon.p_catch_system_errors.value = p != "no";
        
        result = ex_mon.execute( 
            ::riakboost::unit_test::callback0<int>( cpp_main_caller( cpp_main, argc, argv ) ) );
        
        if( result == 0 )
            result = ::riakboost::exit_success;
        else if( result != ::riakboost::exit_success ) {
            std::cout << "\n**** error return code: " << result << std::endl;
            result = ::riakboost::exit_failure;
        }
    }
    catch( ::riakboost::execution_exception const& exex ) {
        std::cout << "\n**** exception(" << exex.code() << "): " << exex.what() << std::endl;
        result = ::riakboost::exit_exception_failure;
    }
    catch( ::riakboost::system_error const& ex ) {
    ...
    if( result != ::riakboost::exit_success ) {
        ...
        ::riakboost::unit_test::const_string p( std::getenv( "BOOST_PRG_MON_CONFIRM" ) );
        if( p != "no" ) { 
...
{
    return ::riakboost::prg_exec_monitor_main( &cpp_main, argc, argv );
}
