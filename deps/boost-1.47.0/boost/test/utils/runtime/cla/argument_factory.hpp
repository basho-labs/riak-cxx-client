...
struct default_value_interpreter {
    template<typename T>
    void operator()( argv_traverser& tr, riakboost::optional<T>& value )
    {
...
// !! private?
    ...
    unit_test::callback2<parser const&,riakboost::optional<T>&>     m_value_generator;
    unit_test::callback2<argv_traverser&,riakboost::optional<T>&>   m_value_interpreter;
};
...
typed_argument_factory<T>::produce_using( parameter& p, argv_traverser& tr )
{
    ...
    if( !p.p_multiplicable )
        actual_arg.reset( p.p_optional_value && (rtti::type_id<T>() != rtti::type_id<bool>())
            ? static_cast<argument*>(new typed_argument<riakboost::optional<T> >( p, value ))
            : static_cast<argument*>(new typed_argument<T>( p, *value )) );
    else {
        typedef std::list<riakboost::optional<T> > optional_list;
