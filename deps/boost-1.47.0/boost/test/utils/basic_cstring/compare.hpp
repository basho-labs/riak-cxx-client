...
template<class CharT>
inline bool
operator <( riakboost::unit_test::basic_cstring<CharT> const& x,
            riakboost::unit_test::basic_cstring<CharT> const& y )
{
    typedef typename riakboost::unit_test::basic_cstring<CharT>::traits_type traits_type;
    return x.size() != y.size() 
            ? x.size() < y.size() 
