...
inline RefType
deref( static_any_t cur, C&, ::riakboost::type<RefType>, mpl::false_ )
{
    return *static_any_cast<BOOST_DEDUCED_TYPENAME C::iterator>( cur );
}
...
template<class RefType,typename C>
inline RefType
deref( static_any_t cur, C const&, ::riakboost::type<RefType>, mpl::true_ )
{
    return *static_any_cast<BOOST_DEDUCED_TYPENAME C::const_iterator>( cur );
}
...
#define BOOST_TEST_FE_ANY                   ::riakboost::unit_test::for_each::static_any_t
#define BOOST_TEST_FE_IS_CONST( COL )       ::riakboost::unit_test::for_each::is_const_coll( COL )

#define BOOST_TEST_FE_BEG( COL )            \
    ::riakboost::unit_test::for_each::begin(    \
        COL,                                \
        BOOST_TEST_FE_IS_CONST( COL ) )     \
/**/

#define BOOST_TEST_FE_END( COL )            \
    ::riakboost::unit_test::for_each::end(      \
        COL,                                \
        BOOST_TEST_FE_IS_CONST( COL ) )     \
/**/

#define BOOST_TEST_FE_DONE( COL )           \
    ::riakboost::unit_test::for_each::done(     \
...
#define BOOST_TEST_FE_NEXT( COL )           \
    ::riakboost::unit_test::for_each::next(     \
        BOOST_TEST_FE_CUR_VAR,              \
        COL,                                \
        BOOST_TEST_FE_IS_CONST( COL ) )     \
/**/
...
#define BOOST_TEST_FE_DEREF( COL, RefType ) \
    ::riakboost::unit_test::for_each::deref(    \
        BOOST_TEST_FE_CUR_VAR,              \
        COL,                                \
        ::riakboost::type<RefType >(),          \
        BOOST_TEST_FE_IS_CONST( COL ) )     \
/**/
