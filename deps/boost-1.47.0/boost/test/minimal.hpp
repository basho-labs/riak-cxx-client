...
#define BOOST_CHECK(exp)       \
  ( (exp)                      \
      ? static_cast<void>(0)   \
      : riakboost::minimal_test::report_error(#exp,__FILE__,__LINE__, BOOST_CURRENT_FUNCTION) )

#define BOOST_REQUIRE(exp)     \
  ( (exp)                      \
      ? static_cast<void>(0)   \
      : riakboost::minimal_test::report_critical_error(#exp,__FILE__,__LINE__,BOOST_CURRENT_FUNCTION))

#define BOOST_ERROR( msg_ )    \
        riakboost::minimal_test::report_error( (msg_),__FILE__,__LINE__, BOOST_CURRENT_FUNCTION, true )
#define BOOST_FAIL( msg_ )     \
        riakboost::minimal_test::report_critical_error( (msg_),__FILE__,__LINE__, BOOST_CURRENT_FUNCTION, true )
...
typedef riakboost::unit_test::const_string const_string;
...
report_critical_error( const char* msg, const char* file, int line, const_string func_name, bool is_msg = false )
{
    ...
    throw riakboost::execution_aborted();
}
...
{
    using namespace riakboost::minimal_test;

    try {
        ::riakboost::execution_monitor ex_mon;
        int run_result = ex_mon.execute( caller( argc, argv ) );

        BOOST_CHECK( run_result == 0 || run_result == riakboost::exit_success );
    }
    catch( riakboost::execution_exception const& exex ) {
        if( exex.code() != riakboost::execution_exception::no_error )
    ...
    if( riakboost::minimal_test::errors_counter() != 0 ) {
        std::cerr << "\n**** " << errors_counter()
                  << " error" << (errors_counter() > 1 ? "s" : "" ) << " detected\n";

        return riakboost::exit_test_failure;
    }

    std::cout << "\n**** no errors detected\n";
    
    return riakboost::exit_success;
}
